...
# Mock supabase client globally to bypass network check during dry run
import db
...
html_path = os.path.abspath(os.path.join(backend_dir, "static", "dry_run_captcha_test.html"))
...
def get_table_mock(table_name):
    ...
    # Define execute side-effects based on table queried
...
# Monkey-patch BrowserContext to auto-simulate user interaction on the test page
...
async def auto_solve_page(page):
    ...
    try:
        ...
        if "dry_run_captcha_test" in current_url:
            ...
            # Click the checkbox inside the captcha widget simulation
...
# Now import the playwright bot task and execute
from utils.playwright_bot import submit_application_task

async def main():
    print("==================================================================")
    print("           PLAYWRIGHT CAPTCHA SOLVING DRY RUN TEST                ")
    print("==================================================================")
    print(f"Loading local mock portal: {file_url}")
    print("Launching Chromium browser in HEADED mode...")
    print("The test harness will automatically click the checkbox and submit")
    print("the form in 4 seconds to simulate manual resolution.")
    print("==================================================================")
